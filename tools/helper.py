...
def semi_loss( z1: torch.Tensor, z2: torch.Tensor,  tau = 0.5):
...
def network_forward_train(EnhNet_model,base_model, PSNet_model, decoder, regressor_delta, pred_scores,
    ...
    total_fea = base_model(total_video)

    transits_pred = PSNet_model(total_fea)
    # print(transits_pred.size())  # torch.Size([4, 16, 96, 2])  torch.Size([16, 96, 2])
    # import sys
    # sys.exit(0)
    ...
    u_fea_96_1 = total_fea[:total_fea.shape[0] // 2].transpose(1, 2)
    u_fea_96_2 = total_fea[total_fea.shape[0] // 2:].transpose(1, 2) # 8, dim, 96

    # add model for enhance time info in segments
    # print(u_fea_96_1.size())
    # u_fea_96_1 = EnhNet_model(u_fea_96_1).permute(0, 2, 1)
    # u_fea_96_2 = EnhNet_model(u_fea_96_2).permute(0, 2, 1)
    # print(u_fea_96_1.size())

    # interpolate to fix_size
    # seg_i: 2bs,fix_size,dim
    ...
    ret_list = []
    for bs in range(video_1_segs.shape[0]):
        for i in range(args.step_num):
            l1 = semi_loss(video_1_segs[bs, i * args.fix_size:(i + 1) * args.fix_size, :],
                           video_2_segs[bs, i * args.fix_size:(i + 1) * args.fix_size, :],)
            l2 = semi_loss(video_2_segs[bs, i * args.fix_size:(i + 1) * args.fix_size, :],
                           video_1_segs[bs, i * args.fix_size:(i + 1) * args.fix_size, :],)
            # print(l1)
            ret = (l1 + l2) * 0.5
            ret_list.append(ret)
    # print(ret_list)
    loss_cont = torch.cat(ret_list).mean() * 10
    # print(loss_cont)
    ...
    loss = loss_aqa + loss_tas + loss_cont
    loss.cuda().backward()
    optimizer.step()
    ...
    if batch_idx % args.print_freq == 0:
        print('[Training][%d/%d][%d/%d] \t Batch_time: %.2f \t Batch_loss: %.4f \t '
              'lr1 : %0.5f \t lr2 : %0.5f' 
              '\t l_aqa : %0.5f \t l_ce : %0.5f \t l_cont : %0.5f '
              % (epoch, args.max_epoch, batch_idx, batch_num, batch_time, loss.item(),
                 optimizer.param_groups[0]['lr'], optimizer.param_groups[1]['lr'],
                 loss_aqa, loss_tas, loss_cont))


def network_forward_test(EnhNet_model,base_model, PSNet_model, decoder, regressor_delta, pred_scores,
    ...
    for video_2, label_2_score, label_2_tas in zip(video_2_list, label_2_score_list, label_2_tas_list):
        ...
        # label_1_tas 未知
        label_1_tas = torch.zeros(label_2_tas.size()).cuda()
...
def save_checkpoint(EnhNet_model,base_model, psnet_model, decoder, regressor_delta, optimizer, epoch,
                    epoch_best_aqa, rho_best, L2_min, RL2_min, prefix, args):
    torch.save({
        'EnhNet_model':EnhNet_model.state_dict(),
